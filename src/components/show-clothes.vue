          ...
          >
            <td class="end-col" width="20">
              ...
              <span
                class="icon-btn"
                :style="{ display: 'none' }"
                v-on:click="deleteClothing(clothing.id, clothing.userid)"
                v-show="updateId === clothing.id"
                ><svg
            ...
            <td>
              ...
              <span v-show="updateId === clothing.id">
                <select
                  class="editField"
                  :style="{ width: clothing.type.length + 3 + 'ch' }"
                  v-model="clothing.type"
                >
            ...
            <td>
              ...
              <span v-show="updateId === clothing.id">
                <input
                  class="editField"
                  :style="{ width: clothing.article.length + 'ch' }"
                  type="text"
                  v-model="clothing.article"
                />
              </span>
            </td>
            <td>
              ...
              <span v-show="updateId === clothing.id">
                <input
                  class="editField"
                  :style="{ width: clothing.colour.length + 'ch' }"
                  type="text"
            ...
            <td>
              <span v-on:click="washClothing(clothing.id, !clothing.wash)"
                ><input type="checkbox" :checked="clothing.wash"
              /></span>
            </td>
            <td class="end-col" width="20">
              <span
                class="icon-btn"
                v-on:click="setUpdateId(clothing, clothing.id, clothing.userid)"
                v-show="hoverId === clothing.id && updateId != clothing.id"
                ><svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="#8D8D8D"
                  width="12"
              ...
              <span
                class="icon-btn"
                v-on:click="cancelUpdateClothing(clothing)"
                v-show="updateId === clothing.id"
                ><svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="#FF0000"
