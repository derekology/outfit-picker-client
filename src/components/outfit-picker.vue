<template>
  <div>
    <p>
      <label class="picklist-label" for="weatherLocation">Forecast for</label
      ><br />
      <select
        class="selectBox"
        id="weatherLocation"
        name="weatherLocation"
        :style="{ width: weatherLocation.length + 3 + 'ch' }"
      ...
      <br /><br /><span
        class="forecast"
        v-show="forecastTemp && forecastWeather"
        >{{ forecastTemp }}°C | {{ forecastWeather }}</span
      >
    </p>
    <br />
    <button
      @click="chooseClothes(weatherLocation), (button_text = 'Choose again!')"
    >
      {{ button_text }}
    </button>
    <p>
    ...
    <p v-if="!this.$store.state.user.data">
      <em
        >**Using demo clothing. Go to the
        <router-link to="/closet">Closet</router-link> to add your own
        clothes.</em
      >
    </p>
  </div>
...
export default {
  ...
  methods: {
    ...
    checkForecast: function (cityName) {
      ...
      if (
        this.weatherLastLocation != this.weatherLocation ||
        this.weatherLastChecked < twoHoursAgo ||
        !this.forecastTemp ||
    ...
    setWeight: function (forecastTemp) {
      if (forecastTemp > 20) {
    ...
    sortClothing: function () {
      ...
      for (let i = 0; i < clothes.length; i++) {
        if (clothes[i].type == "Top") {
          tops.push(
            `${clothes[i].weight} ${clothes[i].colour} ${clothes[i].article}`
          );
