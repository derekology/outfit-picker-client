          ...
          <table>
            ...
            <tbody>
              ...
              >
                ...
                <td class="end-col" width="20">
                  <span class="icon-btn" v-on:click="setUpdateId(clothing, clothing.id, clothing.userid)" v-show="hoverId === clothing.id && updateId != clothing.id"><svg xmlns="http://www.w3.org/2000/svg" fill="#8D8D8D" width="12" height="12" viewBox="0 0 24 24"><path d="M14.078 4.232l-12.64 12.639-1.438 7.129 7.127-1.438 12.641-12.64-5.69-5.69zm-10.369 14.893l-.85-.85 11.141-11.125.849.849-11.14 11.126zm2.008 2.008l-.85-.85 11.141-11.125.85.85-11.141 11.125zm18.283-15.444l-2.816 2.818-5.691-5.691 2.816-2.816 5.691 5.689z"/></svg> </span>
                  <span class="icon-btn" v-on:click="updateClothing(clothing.id, clothing.type, clothing.article, clothing.colour, clothing.weight, clothing.userid)" v-show="updateId === clothing.id"><svg xmlns="http://www.w3.org/2000/svg" fill="#00db3a" width="12" height="12" viewBox="0 0 24 24"><path d="M9 22l-10-10.598 2.798-2.859 7.149 7.473 13.144-14.016 2.909 2.806z"/></svg></span>
                  <span class="icon-btn" v-on:click="cancelUpdateClothing(clothing)" v-show="updateId === clothing.id"><svg xmlns="http://www.w3.org/2000/svg" fill="#FF0000" width="12" height="12" viewBox="0 0 24 24"><path d="M23 20.168l-8.185-8.187 8.185-8.174-2.832-2.807-8.182 8.179-8.176-8.179-2.81 2.81 8.186 8.196-8.186 8.184 2.81 2.81 8.203-8.192 8.18 8.192z"/></svg></span>
                </td>
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  updated: function () {
    ...
    methods: {
      ...
      updateClothing(id, type, article, colour, weight, userid) {        
        if (userid === 'demo') {
          alert('Cannot edit demo clothing.') }
        else {
